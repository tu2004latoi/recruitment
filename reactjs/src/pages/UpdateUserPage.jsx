...
const UpdateUserPage = () => {
  ...
  useEffect(() => {
    const fetchUser = async () => {
      const res = await authApis().get(endpoints.userDetail(userId));
      setUser({
        ...res.data,
    ...
    const fetchApplicant = async () => {
      if (user.role === "APPLICANT") {
        try {
          ...
          const educationRes = await authApis().get(endpoints.applicantEducations(res.data.userId || res.data.id || userId));
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await authApis().patch(endpoints.updateUser(userId), formData);

      // Nếu role là ADMIN, cập nhật admin sau khi đã cập nhật user
      if (user.role === "ADMIN") {
        const adminUpdateData = {
          userId: parseInt(userId)
        };

        await authApis().patch(endpoints.updateAdmin(userId), adminUpdateData, {
          headers: {
      ...
      if (user.role === "RECRUITER") {
        ...
        await authApis().patch(endpoints.updateRecruiter(userId), recruiterUpdateData, {
          headers: {
            "Content-Type": "application/json",
          },
        });
      ...
      if (user.role === "APPLICANT") {
        ...
        const applicantRes = await authApis().patch(endpoints.updateApplicant(userId), applicantUpdateData, {
          headers: {
            "Content-Type": "application/json",
          },
        });
  ...
  return (
    <div className="max-w-3xl mx-auto p-8 bg-gradient-to-br from-blue-100 via-white to-blue-50 rounded-3xl shadow-2xl border border-blue-200 animate-fade-in">
      <h1 className="flex items-center justify-center gap-3 text-4xl font-extrabold mb-10 text-blue-800 text-center tracking-wide drop-shadow-lg">
        <FaUserEdit className="text-blue-500" /> Cập Nhật Người Dùng
      </h1>
      <form onSubmit={handleSubmit} className="space-y-8">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          {/* Password */}
          <div>
            <label className="block mb-1 font-semibold text-gray-600 text-sm">Mật khẩu</label>
            <input
              type="password"
              name="password"
              value={user.password || ""}
